...
link(1) = synapse(lif(1),lif(3),0.005);           % Create Synapse 1->2 
link(2) = synapse(lif(1),lif(4),0.005);           % Create Synapse 1->4 
link(3) = synapse(lif(3),lif(2),0.003);           % Create Synapse 3->2 
link(4) = synapse(lif(4),lif(2),0.003);           % Create Synapse 4->2
stim = create('stimulator',2,{1e-7, 'step', [10e-3 60e-3]});
rec = create('recorder',4);
connect(rec,lif,'Vm');
connect(stim,[lif(4), lif(3)]);
